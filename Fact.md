...
- [2026-09-06 v8.35 协调协议二轮 + 摄像头 Agent 排查] 用户四条裁决/指令：①「每个专家的情况要汇总到看板，
  具体来说不是手上在执行什么命令而是准备做什么事情」→ 专家/子Agent uid 加身份后缀逐个成行（agent_uid），
  task=本轮任务意图，嗅探只登记资源不再覆盖「接下来」；②叫停=右下角停止键，不做跨进程通道（看板只做定位）；
  ③「包要发消息的」→ 冲突新增 coordination 事件给人类（gui chat 气泡 / CLI 黄字 / 专家经 sub_event，
  同组键只报一次）；④「看看那个独立的可以开摄像头的Agent」→ 全仓零摄像头代码（无 cv2/VideoCapture/
  getUserMedia/摄像头字样，Future.md 无立项），唯一独立 Agent=悬浮助手「小龙」（voice_pet_host.py +
  voice-pet.js，纯 Web Speech 语音+任务 CRUD，无 getUserMedia）；已按字面过查其代码（宿主端白名单/边界/
  线程池 IO 干净，前端 appendMessage 走 textContent 无注入面，模式判定失败有兜底，进度条数字归一加固）；
  摄像头能力不存在，是否给小龙加摄像头（getUserMedia+多模态，涉隐私）待用户表态。备份=基线提交 c42b909；
  新增 11 条断言；index.html ?v=8.35.0（voice-pet.js 数字归一触发）。
- [2026-09-06 v8.36 摄像头交互 + 缺口盘点] 用户拍板「我说的就是希望有这个，允许打开摄像头交互——硬件短接
  烧录时腾不出手，要么说话要么摄像头看到短接就开始跑」，并要求盘点 Design 承诺未实现项。落地：
  ①小龙摄像头交互（voice-pet.js：用户点「看」/说「看一下」→ 抓一帧 768px JPEG → 多模态消息走既有
  /api/llm/chat 代理 → 视觉判定「条件就绪，可以执行」→ 自动进 decideAndProcess 决策链接 agent_loop
  真正执行；隐私=流显式建立、按需抓一帧、关面板即断；需视觉模型+HTTPS/localhost）。②缺口盘点结果：
  变更栏引用形式（预览/源码/仅文件）+ 多图扑克牌 ✓ v8.29 已实现（chat.js）；云端交付纪律提示词 ✓ 已实现
  （agent.js:137）；F12 操作器 ✓ v8.14 已实现；模式预设 CLI --preset ✓ 已实现、本轮补 GUI 下拉
  （决策 146）；摄像头 ✓ 本轮新建（决策 145）。引用的五项承诺全部落位，无未实现遗留。
  index.html ?v=8.36.0；设计决策 145/146 入 Design §6.19。备份事实：v8.35 与 v8.36 两轮改动均未单独
  建基线提交（本轮漏打轮前快照），统一叠在基线 c42b909（v8.34 态）之上待验货；回滚到 v8.34 用 c42b909。
